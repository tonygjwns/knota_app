...
export default function Bookmarks() {
  ...
  const loadData = async () => {
    setLoading(true);
    const [bmarks, allTools, recentAttempts] = await Promise.all([
      ...
      <div className="space-y-5 pb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">나중에 다시 공부하려고 표시한 매듭들이에요</p>
          </div>
        </div>
        ...
        {bookmarks.length === 0 ? (
          <div className="flex flex-col items-center gap-4 py-16 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">매듭 보강 중에 ⭐ 버튼으로 추가할 수 있어요</p>
            </div>
                ...
                <Card key={bookmark.id} className="p-4 space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-foreground">
                        {tool?.name || bookmark.tool_id}
                      </p>
                      {tool?.goal && (
                        <p className="text-sm text-muted-foreground mt-0.5">{tool.goal}</p>
                      )}
                      {tool?.description && (
                        <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{tool.description}</p>
                      )}
                      {contextDate && (
                        <p className="text-xs text-muted-foreground mt-1">{contextDate}에 풀던 문제에서 추가했어요</p>
                      )}
                      {bookmark.note && (
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleUnbookmark(bookmark)}
